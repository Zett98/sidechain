...
let test_add_pair () =
  ...
  let result =
    Vm_test.execute_ast_exn 2901 (Pair (Int64 23L, Int64 28L)) script
  in
  let expected_value =
...
let test_if_expr () =
  ...
  (* Check increment *)
  let result =
    Vm_test.execute_ast_exn 4001 (Pair (Int64 1L, Int64 51L)) script
  in
  let expected_value =
  ...
  (* Check decrement *)
  let result =
    Vm_test.execute_ast_exn 4001 (Pair (Int64 0L, Int64 33L)) script
  in
  let expected_value =
...
let test_lambda () =
  ...
  (* Check increment *)
  let result =
    Vm_test.execute_ast_exn 6501 (Pair (Int64 1L, Int64 99L)) script
  in
  let expected_value =
  ...
  (* Check decrement *)
  let result =
    Vm_test.execute_ast_exn 6501 (Pair (Int64 0L, Int64 33L)) script
  in
  let expected_value =
    Vm_test.compile_value_exn (Gas.make ~initial_gas:101) (Int64 32L)
  in
