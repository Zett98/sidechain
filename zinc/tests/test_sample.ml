...
let%expect_test _ =
  ...
  let run_resutl = eval' (module Executor) (initial_state zinc) ~debug:true in
  let run_resutl2 = eval (module Executor) (initial_state zinc) in
  assert (Types.Interpreter_output.equal run_resutl run_resutl2) ;
